...
//  //  return decodedToken?.sub || null; // 'sub' es el claim que contiene el ID del usuario
//  //}
...
export class SeguridadService {
  private http = inject(HttpClient);
  private urlBase = 'https://localhost:7225/api/usuarios';
  private readonly llaveToken = 'token';
  private readonly llaveExpiracion = 'token-expiracion'
  private readonly llaveIdUsuario = 'idUsuario'; // Agregamos una clave para el ID del usuario

  login(credenciales: CredencialesUsuarioDTO): Observable<RespuestaAutenticacionDTO> {
    return this.http.post<RespuestaAutenticacionDTO>(`${this.urlBase}/login`, credenciales).pipe(
      tap((respuestaAutenticacion) => {
        this.guardarToken(respuestaAutenticacion);
        this.guardarIdUsuario(respuestaAutenticacion.userId); // Guardamos el ID del usuario
      })
    );
  }

  private guardarToken(respuestaAutenticacion: RespuestaAutenticacionDTO) {
    localStorage.setItem(this.llaveToken, respuestaAutenticacion.token);
  }

  private guardarIdUsuario(idUsuario: string) {
    localStorage.setItem(this.llaveIdUsuario, idUsuario); // Guardamos el ID del usuario
  }

  obtenerIdUsuario(): string | null {
    return localStorage.getItem(this.llaveIdUsuario); // Obtenemos el ID del usuario
  }
  obtenerUsuariosPaginado(paginacion: PaginacionDTO): Observable<HttpResponse<UsuarioDTO[]>> {
      let queryParams = construirQueryParams(paginacion);
      return this.http.get<UsuarioDTO[]>(`${this.urlBase}/ListadoUsuarios`, {params: queryParams, observe: `response` });
